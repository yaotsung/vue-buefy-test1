  ...
  <div class="about">
    <div>
      <section>
        ...
        <b-select placeholder="Select a block" v-model="block" v-on:input="findlocation()">
...
export default {
  ...
  methods: {
      ...
      this.axios.get(this.api).then((res) => {
        for (let n = 0; n <= 438; n++) {
          if (this.city == res.data['records']['location'][n]['parameter']['0']['parameterValue']){
            if (this.blocks.indexOf(res.data['records']['location'][n]['parameter']['2']['parameterValue']) == -1)
            this.blocks.push(res.data['records']['location'][n]['parameter']['2']['parameterValue'])}
        }
      })
    },
    findlocation(){
      ...
      this.axios.get(this.api).then((res) => {
      for (let n = 0;n <= 438; n++){
        if (this.block == res.data['records']['location'][n]['parameter']['2']['parameterValue']){
          if (this.city == res.data['records']['location'][n]['parameter']['0']['parameterValue']){
            this.locations.push(res.data['records']['location'][n]['locationName'])
          }
        }
      }
      })
    ...
    output(){
      ...
      this.axios.get(this.api).then((res) => {
        for (let n = 0;n <= 438;n++){
          if (this.location == res.data['records']['location'][n]['locationName']){
            ...
            this.temp = res.data['records']['location'][n]['weatherElement']['3']['elementValue']
          }
        }
      })
    }
